...
def get_lv_info(lvPath):
	lvInfo = {}
	lvCfgParser = ConfigParser.ConfigParser()
	cfgFile = os.path.join(lvPath,'buildInfo.txt')
	lvCfgParser.read(cfgFile)
	lvInfo['lvVersion'] = lvCfgParser.get('Build Information', 'lvVersion')
	lvInfo['lvAPIVersion'] = lvCfgParser.get('Build Information','lvAPIVersion')
	return lvInfo
...
def construct_record_dict(stackUrl):
	...
	lvPath = installerLocationDict['LV_Core']                                                      
	lvInfoDict = get_lv_info(lvPath)
